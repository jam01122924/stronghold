...
class TacticsStage extends React.Component {
  ...
  selectHero(hero) {
    ...
    if(heroData) {
      ...
      heroData.battleStatus.facing = 'top';
    }
  ...
  initMonster() {
    ...
    // right now, just assume every enemy tactic grid is open:
    let openGrid = []
    for(let i=0; i<this.props.battleMapDatas.data.length; i++) {
      for(let j=0; j<this.props.battleMapDatas.data[i].length; j++) {
        if(this.props.battleMapDatas.data[i][j].battle.tactics==='enemy') {
          openGrid.push({x: j, y: i});
        }
      }
    }
    ...
    for(let i=0; i<monsterArray.length; i++) {
      if(openGrid.length) {
        ...
        monsterArray[i].battleStatus = {
          ...
          position: {x: openGrid[randIndex].x, y: openGrid[randIndex].y},
          targetPosition: {x: null, y:null},
          target: null,
          path: [],
          facing: 'bottom', // left, right, top, bottom
        };
        openGrid.splice(randIndex, 1);
      }
    }
    console.log(monsterArray);
    ...
    if(monsterArray) {
      ...
      // TODO: decide who's round first:
      this.props.dispatch(battleActions.setBattleStatus('enemyRound'));
  ...
  confirm() {
    ...
    let battleMapData = battleServices.endTactics(this.props.battleMapDatas.data);
